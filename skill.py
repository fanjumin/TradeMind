"""
TradeMind 增强版 - 进化型股票分析师 Skill
集成了原始 TradeMind 功能与 Alpha Evol 进化可视化特性
"""
import json
import math
import os
...
class TradeMindSkill:
    ...
    def analyze_stock(self, symbol):
        """增强的股票分析方法，返回进化友好的 JSON"""
        try:
            df = get_price_data(symbol)
            if df is None or df.empty:
                return json.dumps({"error": f"无法获取股票 {symbol} 的数据"}, ensure_ascii=False)
            ...
            # K-line data (最近30日)
            df_kline = get_price_data(symbol, days=30)
            kline = []
            if df_kline is not None and not df_kline.empty:
                for _, row in df_kline.iterrows():
                    time_val = row.name if hasattr(row, 'name') else row.get('date', '')
            ...
            depth = sum([
                ...
                1 if fund_total and float(fund_total) > 10000000 else 0,
            ...
            analysis_result = {
                ...
                "current_price": float(latest.get("close", 0)) if latest and latest.get("close") else 0.0,
                "recommendation": signal,
                "confidence": min(0.95, 0.5 + abs(score) * 0.01),
                "risk_level": "LOW" if score > 80 else "MEDIUM" if score > 40 else "HIGH",
                "summary": f"{basic.get('name', symbol)}：{get_signal_cn(signal)}，价格{latest.get('close', 0) if latest and latest.get('close') else 0:.2f}",
                "key_points": basic_reasons[:3] if basic_reasons else [],
                "buy_point": {"price": levels["support"], "reason": "支撑位反弹"} if levels and levels.get("support") else {"price": 0, "reason": "-"},
                "sell_point": {"price": levels["resistance"], "reason": "前高阻力"} if levels and levels.get("resistance") else {"price": 0, "reason": "-"},
                "kline": kline
            }
            ...
            return json.dumps({"error": str(e), "skill": "TradeMind"}, ensure_ascii=False)
...
def run(symbol):
    skill = TradeMindSkill()
    result = skill.analyze_stock(symbol)
    return result
